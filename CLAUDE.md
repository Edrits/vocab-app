...
- **`vocab.json`** — the entire database: a flat array of word entries. Each entry's schema and the quality bar are documented in `DAILY_FETCH.md`. **Don't read it wholesale or hand-edit it — it grows daily.** To see what's in the deck, extract keys via a shell one-liner (see `DAILY_FETCH.md`); to add words, write the new entries to a scratch file and run `python3 scripts/add_words.py <file>`, which validates, dedupes on `id`/`hanzi`, appends, and prints deck stats.
  - **Sourcing is mandatory.** Every entry carries a `sources` field — a JSON array of one or more objects `{ "url", "note", "lang" }` — that backs up the term and its example. `url` is an http(s) link to the evidence; `note` is a short human citation (source name + what it is), in **Mandarin or English**; `lang` is `"zh"` or `"en"`. Multiple sources are fine (list them all). Sourcing is **not** rendered on the study card — it exists to make each entry verifiable and to be extracted on export. `add_words.py` (and `scripts/backfill_sources.py`, which patches `sources` onto existing entries in place) reject any malformed `sources`. (Note: entries added before this rule are being backfilled — see the plan memory.)
- **`reference.json`** — read-only reference material, deliberately **separate from `vocab.json`**: two collections (`ran`, `utterances`), each with a `title`, `subtitle`, ordered `groups`, and `items` (`hanzi`, `pinyin`, `gloss`, `group`, `note`, `example`, `example_pinyin`, `example_translation`, plus an optional `blend` note on how a phrase slurs together in fast speech, rendered as a "Said fast" row). Loaded lazily the first time a reference mode is opened. These entries never enter the review queue and never touch SRS state; the daily netizen fetch does not own this file. **Add to it with `python3 scripts/add_utterances.py <file> [collection]`** (validates the schema, checks each `group` exists, dedupes on `hanzi`; `collection` defaults to `utterances`, pass `ran` for the 然-combos set) rather than hand-editing. New entries carry a `sources` array of `{url, note, lang}` usage evidence (the 51 original hand-added entries predate this and have none); `sources` is never rendered in the app.
- **`index.html`** — the whole app (markup, CSS, and vanilla JS inline). Four tabs: **Study** (SM-2-lite flashcards), **Quiz** (multiple-choice rounds, XP only — never touches the SRS schedule), **Browse** (search/sort/filter list), and **Me** (netizen rank + XP, 打卡 streak heatmap, word of the day, mastery). Two further **reference modes** sit behind the ☰ sidebar rather than the tab bar — **然 combos** (`mode: ran`) and **口头禅 · Utterance glossary** (`mode: talk`), both read-only and fed from `reference.json`. `render()` dispatches to `renderStudy`/`renderQuiz`/`renderBrowse`/`renderMe` by the current `mode`, or to `renderReference()` when `REF_MODES` maps that mode to a collection. Pinyin renders tone-coloured via `tonePinyin()`; audio uses the browser's built-in zh-CN speech synthesis. Category styling keys off `cat-<category>` classes (e.g. chengyu renders gold).
  - **SRS grading** (`grade()`/`nextStep()`) has non-obvious tuning: ease floors at `MIN_EASE` (1.3), a forgotten card keeps half its interval via `LAPSE_KEEP` rather than resetting to zero, and a lapsed-then-relearning card resumes the interval it had at the lapse. `repairState()` is a one-off migration that bumps up cards left at a punishingly low ease by an older, harsher scheduler — read it before touching the grading formula, since a past version of this logic caused reviewed words to resurface too soon.
...
- **`UTTERANCE_FETCH.md`** — the recipe for growing the `utterances` collection in `reference.json`: how to research spoken-discourse expressions that sit *outside* dictionary Mandarin (fillers, tags, reactions, closers), where to find real unscripted usage, and how to source and append them via `add_utterances.py`. Run on demand, not scheduled. Durable spoken habits go here; trending slang stays in `vocab.json`.
