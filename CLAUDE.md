...
- **`vocab.json`** — the entire database: a flat array of word entries. Each entry's schema and the quality bar are documented in `DAILY_FETCH.md`. **Don't read it wholesale or hand-edit it — it grows daily.** To see what's in the deck, extract keys via a shell one-liner (see `DAILY_FETCH.md`); to add words, write the new entries to a scratch file and run `python3 scripts/add_words.py <file>`, which validates, dedupes on `id`/`hanzi`, appends, and prints deck stats.
  - **Sourcing is mandatory.** Every entry carries a `sources` field — a JSON array of one or more strings — that backs up the term and its example. Each string is a citation: a URL, or a short reference/description, in **Mandarin or English**. Multiple sources are fine (list them all). Sourcing is **not** rendered on the study card — it exists to make each entry verifiable and to be extracted on export. `add_words.py` rejects any new entry without at least one non-empty source. (Note: entries added before this rule may lack sources; they are being backfilled — see the plan memory.)
